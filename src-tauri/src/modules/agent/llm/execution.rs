...
use crate::modules::agent::tools::{
    batch_ops, delete_path, list_dir, mkdir, move_path, read_file, write_file, ToolContext,
};
...
pub async fn call_tool(
    ...
    match name {
        "read_file" => {
            read_file(
                ctx,
                serde_json::from_value(args_value).map_err(|_| "参数不合法".to_string())?,
            )
            .await
        }
        "list_dir" => {
        ...
        _ => {
            ...
            if target_server.is_none() {
                for (server_name, mcp_tools) in mcp_tools_map {
                    if let Some(tool) = mcp_tools.iter().find(|t| {
                        t.get("name")
                            .and_then(|v| v.as_str())
            ...
            if let (Some(server_name), Some(real_name)) = (target_server, target_tool_name) {
                ...
                return client.send_request_and_wait(
                    "tools/call",
                    Some(params),
            ...
            let available: Vec<String> = mcp_tools_map
                .values()
                .flat_map(|tools| {
