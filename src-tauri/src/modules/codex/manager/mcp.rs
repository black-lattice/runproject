...
use super::tools::{extract_tools, is_initialize_result, is_tools_list_result, select_tool_name};
use super::SESSIONS;
...
pub fn start_mcp_handshake(_app: &AppHandle, session_id: &str) -> Result<(), String> {
    ...
    let init_result =
        connection.send_request_and_wait("initialize", Some(init_params), Duration::from_secs(15));

    if let Err(init_error) = init_result {
        println!(
            "[CODEX] MCP initialize failed, trying tools/list fallback: {}",
            init_error
        );
        let tools_result =
...
pub fn handle_mcp_message(app: &AppHandle, session_id: &str, message: &CodexIncomingMessage) {
    match message {
        CodexIncomingMessage::Response(response) => match response {
                    ...
                    if let Some(msg) = params.get("msg") {
                        if let Some(msg_type) = msg.get("type").and_then(|v| v.as_str()) {
                            if msg_type == "session_configured" {
                                ...
                                {
                                    println!(
                                        "DEBUG: Session configured with ID: {}",
                                        new_session_id
                                    );
                                    if let Ok(mut sessions) = SESSIONS.lock() {
                                        if let Some(session) = sessions.get_mut(session_id) {
