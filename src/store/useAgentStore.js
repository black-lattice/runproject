...
export const useAgentStore = create(
	persist(
		(set, get) => ({
			...
			isStarting: false,
			isSending: false,
			savedModels: [], // Persisted list of models
			...
			startSession: async ({ sessionId, title, workspace }) => {
				...
				// If no workspace provided, just create the session entry and wait
				if (!workspace) {
					set(state => ({
						sessions: [
							...state.sessions,
				...
				try {
					const subscription = isCodexProvider
						? await startCodexSession({
							...
							onFileChange: change =>
									get().handleCodexEvent(sessionId, change)
								})
			...
			stopSession: async sessionId => {
				...
				updateSessionWorkspace: async (sessionId, newWorkspace) => {
					const folderName = newWorkspace.trim().split('/').filter(Boolean).pop() || '工作区';
					const session = get().sessions.find(s => s.id === sessionId);
					
					if (!session) return;
			
					const provider = get().settings?.provider || session.provider;
					set(state => ({
						sessions: state.sessions.map(s =>
							s.id === sessionId
								? {
										...
										title: folderName,
										provider, // Ensure provider is synced
										lastUpdated: Date.now(),
										// If it was pending, update status to starting
										status: s.status === 'pending_workspace' ? 'starting' : s.status
									}
								: s
						)
					}));
			
					// If the session was pending workspace, now we start it
					if (session.status === 'pending_workspace') {
						const isCodexProvider = provider === 'codex';
			...
			sendMessage: async ({ sessionId, content }) => {
				...
				try {
					...
					if (session?.provider === 'codex') {
						...
						const model =
							settingsModel === 'codex-cli' ? undefined : settingsModel;
							...
							? {
									...
									pendingActions: [
										...session.pendingActions.filter(
												action => action.callId !== payload.callId
											),
											{
			...
			updateStatus: (sessionId, status) => {
				...
				const error =
					typeof status === 'object'
						? status?.error || status?.payload?.error || null
						: null;
						...
						session.id === sessionId
							? {
									...
									ready:
										...
										(normalized === 'sessionactive' || normalized === 'ready')
											? true
											: session.ready,
									lastUpdated: Date.now()
								}
			...
			updateSavedModel: (id, model) =>
				set(state => ({
					savedModels: state.savedModels.map(m =>
						m.id === id ? { ...m, ...model } : m
					)
				})),
			deleteSavedModel: id =>
		...
		}),
		{
			...
			partialize: state => ({ savedModels: state.savedModels })
		}
	)
);
