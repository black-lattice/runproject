...
function ProjectItem({
	...
	return (
		<div
			className={cn(
				'project-list-item group flex flex-col gap-1 px-2 py-2 rounded-md cursor-pointer transition-all duration-200 border border-transparent select-none relative',
				isSelected
					? 'project-list-item-selected bg-accent/80 text-primary'
			...
			<div className='flex items-center gap-2.5 min-w-0'>
				<div
					className={cn(
						'project-list-item-icon p-1 rounded-md transition-colors flex-shrink-0',
						isSelected
				...
				<Popover
					...
					<PopoverContent>
						<div className='space-y-3'>
							<p className='text-sm text-foreground'>项目标签（逗号分隔）</p>
							<Input
								value={tagDraft}
							...
							<div className='flex items-center gap-2 justify-end'>
								...
								<Button size='sm' className='h-8 px-3' onClick={handleSaveTags}>
									保存
								</Button>
							</div>
						</div>
				...
				{/* 选中状态打钩，或者其他指示 */}
				{/* {isSelected && <Check className="w-4 h-4 text-primary opacity-50" />} */}
			</div>
		</div>
	);
}
