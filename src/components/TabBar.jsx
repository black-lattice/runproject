...
function TabBar() {
	...
	const activeTab =
		Object.values(PAGE_CONFIGS).find(c => c.path === location.pathname)?.id ||
		'welcome';
			...
			<div className='flex gap-1 overflow-x-auto no-scrollbar'>
				{tabs.map(tabId => {
					...
					return (
						...
						>
							{Icon && <Icon className='w-3.5 h-3.5 mr-1.5 opacity-70' />}
							...
							{config.closable && (
								<Button
									...
									size='sm'
									className={cn(
										'ml-2 h-3 w-3 p-0 hover:bg-red-200 hover:text-red-700 rounded-full transition-all duration-200',
										isActive
